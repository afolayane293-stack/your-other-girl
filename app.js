// YOUR OTHER GIRL 💗
// Clean frontend — navigation, customization, chat, study, tools and profile
...
async function sendMessage() {
  ...
  const thinkingMessage = messages
    ? messages.querySelector(".msg.girl:last-child")
    : null;

  try {
    const apiBase =
      window.YOG_API || "https://your-other-girl.onrender.com";
    ...
    const response = await fetch(apiBase + "/api/chat", {
      ...
      body: JSON.stringify({
        ...
        profile: {
          name: state.name || "",
          vibes: state.vibes || [],
          style: Array.isArray(state.style)
            ? state.style[0] || ""
            : "",
          emoji: Array.isArray(state.emoji)
            ? state.emoji[0] || ""
            : "",
          custom: state.custom || ""
        }
    ...
    if (thinkingMessage) {
      thinkingMessage.textContent =
        data.reply ||
        "I'm here, girl 💗 I didn't get a reply yet.";
    }
...
$$("[data-study]").forEach(button => {
  button.addEventListener("click", () => {
    ...
    box.innerHTML = `
      <h3>${escapeHTML(button.dataset.study)} 💗</h3>
      ...
      <button class="primary" id="askStudy">
        Ask my pocket girl →
      </button>
    `;
    ...
    $("#askStudy").addEventListener("click", () => {
      ...
      box.innerHTML = `
        ...
        <p>
          The AI study engine will be connected to the backend next.
        </p>
      `;
    });
  });
...
$$("[data-tool]").forEach(button => {
  button.addEventListener("click", () => {
    ...
    box.innerHTML = `
      ...
      <button class="primary" id="saveToolButton">
        Save
      </button>
    `;
...
$$("[data-problem]").forEach(button => {
  button.addEventListener("click", () => {
    ...
    box.innerHTML = `
      <div class="card">
        <b>${escapeHTML(button.dataset.problem)}</b>
